...
final class OfferForm extends EntityForm {
  public function form(array $form, FormStateInterface $form_state): array {
    ...
    foreach (PaymentGateway::loadMultiple() as $gateway) {
      $gateway_options[$gateway->id()] = $gateway->status()
    ...
    $form['payment_gateway_id'] = ['#type' => 'select', '#title' => $this->t('Payment gateway'), '#description' => $this->t('The only Commerce gateway allowed for this offer. Recurring offers require the PayPal Subscriptions gateway.'), '#options' => $gateway_options, '#default_value' => $offer->getPaymentGatewayId(), '#required' => TRUE];
  ...
  public function validateForm(array &$form, FormStateInterface $form_state): void {
    ...
    $gateway_id = trim((string) $form_state->getValue('payment_gateway_id'));
    $gateway = PaymentGateway::load($gateway_id);
    if (!$gateway) {
      $form_state->setErrorByName('payment_gateway_id', $this->t('The selected Commerce payment gateway does not exist.'));
    }
    elseif ($form_state->getValue('purchase_type') === 'recurring' && $gateway->getPluginId() !== 'paypal_checkout_subscriptions') {
      $form_state->setErrorByName('payment_gateway_id', $this->t('A recurring offer must use a PayPal Subscriptions payment gateway.'));
    }
