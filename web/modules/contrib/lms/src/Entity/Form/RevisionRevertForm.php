...
/**
 * Copied from core as the core one is marked as internal (BC).
 *
 * @todo Track changes in the core class.
 */
abstract class RevisionRevertForm extends ConfirmFormBase implements EntityFormInterface {
  ...
  public function getQuestion() {
    return $this->t('Reverting revision of "@label"', ['@label' => $this->revision->label()]);
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void {
    ...
    $originalRevisionTimestamp = NULL;
    ...
    if ($originalRevisionTimestamp !== NULL) {
    ...
    if ($form_state->getRedirect() === FALSE || $form_state->getRedirect() === NULL) {
  ...
  protected function prepareRevision(RevisionableInterface $revision, FormStateInterface $formState): RevisionableInterface {
    ...
    if ($revision instanceof RevisionLogInterface) {
      ...
      $date = $this->dateFormatter->format($originalRevisionTimestamp);
      $revision
        ->setRevisionLogMessage((string) $this->t('Copy of the revision from %date.', ['%date' => $date]))
  ...
  protected function getBundleLabel(RevisionableInterface $entity): ?string {
    ...
    $label = $bundleInfo[$entity->bundle()]['label'] ?? NULL;
