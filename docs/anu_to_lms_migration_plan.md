...
| `question_short_answer` | `free_text` (or feedback variant) | map prompt + expected guidance; group adjacent short/long-answer lesson or assessment questions into one activity using the LMS `questions` array and name multi-prompt groups `Questions` | P1 |
| `question_long_answer` | `free_text` | map prompt + rubric hints; group adjacent short/long-answer lesson or assessment questions into one activity using the LMS `questions` array and name multi-prompt groups `Questions` | P1 |
